...
def decode_dataset(
    ...
    Returns:
      ...
      Its value is a list of lists. Each list contains two elements:
    ...
    for num, cut in enumerate(cuts):
        ...
        lang_token = sp.encode(LANGTAG[cut.supervisions[0].language])[1]
    ...
    for key, results in results_dict.items():
        ...
        with open(errs_filename, "w") as f:
            wer = write_error_stats(
                f, f"{test_set_name}-{key}", results, enable_log=True, compute_CER=compute_cer
            )
...
def main():
    ...
    params.chunk_size = params.decoding_chunk_size
    params.left_context_frames = params.decoding_left_context_frames
    params.exp_dir = Path(params.exp_dir)
    params.manifest_dir = Path(params.manifest_dir)
    params.res_dir = params.exp_dir / "streaming" / params.decoding_method

    if params.decode_ckpt:
        params.suffix = f"{Path(params.decode_ckpt).stem}"
    elif params.iter > 0:
    ...
    if not params.use_averaged_model:
        if params.decode_ckpt:
            load_checkpoint(f"{params.decode_ckpt}", model)
        elif params.iter > 0:
    ...
    test_cuts = asr_dm.test_cuts()

    for test_set, test_cut in test_cuts.items():
        ...
        language = test_cut.sample(n_cuts=1).supervisions[0].language
        save_results(
            params=params,
